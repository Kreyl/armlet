...
void MassStorage_t::UsbOutTask() {
    ...
    if(Usb.State == usConfigured) {
        // Receive header
        Uart.Printf("1\r");
        uint32_t Rcvd = Usb.PEpBulkOut->ReceiveToBuf((uint8_t*)&CmdBlock, MS_CMD_SZ, TIME_INFINITE);
        Uart.Printf("Rcvd: %u\r", Rcvd);
        if(Rcvd == MS_CMD_SZ) SCSICmdHandler();
    }
}
#endif
...
bool MassStorage_t::CmdWrite10() {
    ...
    while(TotalBlocks != 0) {
        ...
        // Get data from USB
        Rcvd = Usb.PEpBulkOut->ReceiveToBuf(Buf1, BytesToReceive, MS2ST(MS_TIMEOUT_MS));
        ...
        if(TotalBlocks != 0) {
            ...
            BlocksToWrite = BytesToReceive / MMCSD_BLOCK_SIZE;
            // Get data from USB
            Rcvd = Usb.PEpBulkOut->ReceiveToBuf(Buf2, BytesToReceive, MS2ST(MS_TIMEOUT_MS));
